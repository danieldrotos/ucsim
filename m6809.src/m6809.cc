...
cl_m6809::inst_alu(t_mem code)
{
  ...
  switch (code & 0x0f)
      ...
      else
	{
      ...
      if ((code & 0x40) == 0)
	return inst_add16(code, &(reg.X), ~op16, 0, false, true, true);
      else
...
cl_m6809::inst_pg1(t_mem code)
{
  ...
  */
  ...
  switch (cl)
    {
    case 3: // CMPD
      inst_add16(code, &(D), ~op16, 0, false, true, true);
      break;
    case 0xc: // CMPY
      inst_add16(code, &(reg.Y), ~op16, 0, false, true, true);
      break;
    case 0xe: // LDY, LDS
      if ((code & 0x40) == 0)
...
cl_m6809::inst_page2(t_mem code)
{
  ...
  if ((code & 0x0f) == 0x03)
    {
      // CMPU
      inst_add16(code, &(reg.U), ~op16, 0, false, true, true);
      tick(1);
    }
  if ((code & 0x0f) == 0x0c)
    {
      // CMPS
      inst_add16(code, &(reg.S), ~op16, 0, false, true, true);
      tick(1);
    }
